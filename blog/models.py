...
class Produce(models.Model):
    ...
    address = models.CharField(max_length=30,unique=True,null=True)
    phone = models.CharField(max_length=100, blank=True,null=True)
    author = models.CharField(max_length=100, blank=True,null=True) # 대표
    ...
    def get_absolute_url(self):
        return f'/blog/Produce/{self.slug}/'
...
class OptionColors(models.Model):
    name = models.CharField(max_length=50, unique=True)  # 동일한 name을 갖는 카테고리를 만들 수 있다 .
    slug = models.SlugField(max_length=200, unique=True, allow_unicode=True)  # 사람이 읽을 수 있는 텍스트로 고유 URL
    ...
    def get_absolute_url(self):
        return f'/blog/OptionColors/{self.slug}/'
...
class Post(models.Model):
    ...
    # Post 모델에 제조사 추가하기
    Produce = models.ForeignKey(Produce, null=True, blank=True,
                                 on_delete=models.SET_NULL)  # 카테고리를 빈칸으로 두기 (blank = True)

    # option 색상 추가하기 (다대일 관계)
    OptionColors = models.ForeignKey(OptionColors, null=True, blank=True,on_delete=models.SET_NULL)

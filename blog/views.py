...
from .models import Post,Category,Tag, Comment,Produce, OptionColors
...
def delete_comment(request,pk):
    ...
    if request.user.is_authenticated and request.user == comment.author:
        comment.delete()
        return redirect(post.get_absolute_url()) # 지우고 나서 post의 상세페이지로 가야 한다.
    else:
        PermissionDenied
...
class PostUpdate(LoginRequiredMixin,UpdateView):
    model = Post
    fields = ['title',  'price', 'content', 'head_image', 'author', 'category', 'Produce', 'OptionColors']
    ...
    def get_context_data(self, *, object_list=None,**kwargs):
        ...
        context['Ocategories'] = OptionColors.objects.all()
        context['Pcategories'] = Produce.objects.all()
        context['no_category_post_count'] = Post.objects.filter(category=None).count()
        return context

class PostCreate(LoginRequiredMixin, UserPassesTestMixin, CreateView):
    model = Post
    fields = ['title', 'price', 'content', 'head_image', 'author', 'category', 'Produce',
              'OptionColors']  # 태그 입력 가능하게 변경 함 (tags 추가 안함 )

    # 태그 입력 가능하게 변경 함 (tags 추가 안함 )
    #모델명_form.html
...
def category_page(request,slug):
    ...
    return render(request,'blog/post_list.html',{
        ...
        'Ocategories':OptionColors.objects.all(),
        'Pcategories':Produce.objects.all(),
...
def optioncolors_page(request,slug):
    OptionColor = OptionColors.objects.get(slug=slug)

    post_list = OptionColor.post_set.all
...
def produce_page(request,slug):
    produce = Produce.objects.get(slug=slug)

    post_list = produce.post_set.all
...
class ME(ListView):
    ...
    def get_context_data(self, **kwargs):
        context = super(ME,self).get_context_data()
        context['comment_form'] = CommentForm
        context['post']: Post
        return context